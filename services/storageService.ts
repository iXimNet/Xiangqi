...
const safeFetch = async <T>(path: string, init?: RequestInit): Promise<T | null> => {
  try {
...
export const fetchCurrentGame = async (): Promise<GameSession | null> => {
  const res = await safeFetch<ApiResponse<GameSession | null>>('/games/current');
  return res?.game ?? null;
};

const persistGame = async (session: GameSession, method: 'POST' | 'PUT') => {
  await safeFetch<ApiResponse<GameSession>>(method === 'POST' ? '/games' : `/games/${session.id}`, {
    method,
